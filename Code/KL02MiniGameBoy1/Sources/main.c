...
bool RenderLevel(byte *PP, int cx, int t, bool *above, bool *below, int *x, int *y, byte *score, byte *lives)
{
...
bool Game()
{
	...
	while (lives >= 0 && level < levelset[0])
	{
		LoadLevel(level);
		...
		RenderLevel(pic2, cx,t,  &above, &below, &x, &y, &score, &lives);
		...
		while (playing == TRUE)
		{
			...
			if (RenderLevel(pic, cx,t,  &above, &below, &x, &y, &score, &lives) == TRUE)
			{
				level++;
				nextlevel = TRUE;
				playing = FALSE;
	...
	if (level == levelset[0]) return TRUE;
	return FALSE;
}
